...
export default function EditPackagingPage() {
  ...
  const fetchPackaging = async () => {
    try {
      const response = await fetch(`/api/product-info/packaging/${id}`)
      if (!response.ok) throw new Error('Packaging not found')
      const data = await response.json()
      ...
      setFormData({
        ...
        measurementUnit: pkg.measurementUnit || 'Pieces',
        unitsPerQuantity: pkg.unitsPerQuantity?.toString() || '1',
        costPerUnit: pkg.costPerUnit?.toString() || '',
        gstRatePct: pkg.gstRatePct?.toString() || '5',
        hsnCode: pkg.hsnCode || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(result.error || 'Failed to update packaging')
      }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Update packaging details"
        breadcrumbs={[
          { label: 'Dashboard', href: '/dashboard' },
          { label: 'Products', href: '/products' },
          { label: 'Packaging', href: '/products/packaging' },
          { label: formData.pkgSku || 'Edit' },
        ]}
        actions={
